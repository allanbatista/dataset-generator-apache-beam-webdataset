...
mvn compile exec:java \
    ...
    -Dexec.args="--runner=DataflowRunner --project=allanbatista --region=us-east1 --stagingLocation=${STG_PATH} --tempLocation=${TEMP_PATH} --gcpTempLocation=${GCP_TEMP_PATH} --outputDir=${OUTPUT_PATH} --numWorkers=${NUM_WORKER} --maxNumWorkers=${NUM_WORKER}  --autoscalingAlgorithm=NONE --workerMachineType=${MACHINE_TYPE} --diskSizeGb=50 --trainShards=${TRAIN_SHARD} --validShards=${VALID_SHARD} --testShards=${TEST_SHARD} --singleDataset=${SINGLE_DATASET}"
